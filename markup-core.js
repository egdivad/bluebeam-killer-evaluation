export const MARKUP_LABELS={line:"Line",arrow:"Arrow",rectangle:"Rectangle",ellipse:"Ellipse",cloud:"Cloud",polygon:"Polygon",freehand:"Freehand",flag:"Flag",callout:"Callout",legend:"Takeoff Legend"};
...
export function markupListRows(annotations=[],pages=[],valueForItem=item=>item.displayValue||""){
  ...
  return annotations.filter(item=>["markup","highlight","text","replacement","measurement"].includes(item.type)&&!item.deleted).map(item=>({
    id:item.id,visible:item.visible!==false,page:pageById.get(item.pageId)||item.page||1,layer:item.layerName||"No layer",type:item.type==="markup"?(MARKUP_LABELS[item.markupKind]||"Markup"):item.type==="measurement"?`${item.measureKind} measure`:item.type,subject:item.subject||item.text?.slice(0,40)||MARKUP_LABELS[item.markupKind]||item.type,value:item.type==="markup"&&["flag","callout"].includes(item.markupKind)?item.text||"":item.type==="markup"&&item.markupKind==="legend"?item.legendTitle||"Takeoff Summary":valueForItem(item),comment:item.comment||"",status:item.status||"None"
