...
function appendMarkupGraphic(svg,annotation,factor,draft=false){
  ...
  if(!draft){group.dataset.id=annotation.id;group.addEventListener("pointerdown",event=>startMarkupMove(event,annotation.id));group.addEventListener("click",event=>{event.preventDefault();event.stopPropagation();selectMarkup(annotation.id);});const editablePoints=kind==="freehand"?points.map((point,index)=>({point,index})).filter((_,index,list)=>index===0||index===list.length-1):points.map((point,index)=>({point,index}));for(const{point,index}of editablePoints){const marker=createSvgElement("circle",{class:"markup-point",cx:point.x,cy:point.y,r:4});marker.addEventListener("pointerdown",event=>startMarkupPointDrag(event,annotation.id,index));group.append(marker);}}svg.append(group);
}
...
function startMarkupPointDrag(event,id,index){event.preventDefault();event.stopPropagation();const annotation=state.annotations.find(item=>item.id===id);if(!annotation)return;snapshot();markupPointDrag={id,index,pageId:annotation.pageId};event.currentTarget.setPointerCapture?.(event.pointerId);}
...
window.addEventListener("pointerup",()=>{if(!markupPointDrag)return;const pageId=markupPointDrag.pageId;markupPointDrag=null;markChanged();queueThumbnailRefresh(pageId);});
...
function appendMeasurementGraphic(svg,annotation,factor,draft=false){
  ...
  for(const [index,point] of points.entries()){const editable=!draft&&annotation.measureKind!=="calibration",marker=createSvgElement("circle",{class:`measurement-point ${editable?"editable":""}`,cx:point.x,cy:point.y,r:editable&&isSelected(annotation.id)?5:3});marker.style.stroke=lineColor;if(editable)marker.addEventListener("pointerdown",event=>startMeasurementPointDrag(event,annotation.id,index));group.append(marker);}
  ...
  if(!draft){group.dataset.id=annotation.id;group.addEventListener("pointerdown",event=>startMeasurementMove(event,annotation.id));group.addEventListener("click",event=>{event.preventDefault();event.stopPropagation();selectMeasurement(annotation.id);});}
  svg.append(group);
}
...
function addBoxHandles(a,factor){
  ...
  move.type="button"; move.className="box-handle move-handle"; move.textContent="Move"; move.title="Move text box. Hold Shift to lock movement to one axis.";move.setAttribute("aria-label",move.title);
...
function placementMeasurementPoint(point,points,shiftKey){
  ...
  if(shiftKey&&anchor){options.anchor=anchor;options.axis=Math.abs(point.x-anchor.x)>=Math.abs(point.y-anchor.y)?"horizontal":"vertical";}
...
function startMeasurementPointDrag(event,id,index){const annotation=state.annotations.find(item=>item.id===id&&item.type==="measurement");if(!annotation)return;event.preventDefault();event.stopPropagation();snapshot();suppressMeasurementClick=true;measurementPointDrag={id,index,pageId:annotation.pageId};if(state.selectedId!==id)selectMeasurement(id);}
...
window.addEventListener("pointerup",()=>{if(!measurementPointDrag)return;const pageId=measurementPointDrag.pageId;measurementPointDrag=null;updateSnapIndicator(null);markChanged();queueThumbnailRefresh(pageId);setTimeout(()=>{suppressMeasurementClick=false;},0);});
...
function runShortcut(command){
  ...
  if(command==="delete"){$("deleteButton").click();return;}
