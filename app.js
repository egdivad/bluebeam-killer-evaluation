...
import { MARKUP_LABELS, arrowheadGeometry, cloudPath, copyPageItem, defaultFlagPoints, flagPolygonPoints, formatPainterPatch, groupMarkupRows, makeMarkup, markupBounds, markupListRows, rowsToCsv, sortMarkupRows } from "./markup-core.js?v=12";
import { addPdfLibAnnotation } from "./pdf-annotations.js?v=6";
...
async function loadPdf(bytes,name) {
  stopFormatPainter(false);state.bytes=bytes; state.pdf=await pdfjsLib.getDocument({data:bytes.slice()}).promise; state.pages=makeSourcePages(state.pdf.numPages);state.page=1; state.annotations=[]; state.textBlocks=[]; state.measurementScales={};state.selectedId=null;state.selectedIds=[];state.history=[];state.future=[];itemClipboard=null;itemPasteSequence=0;vectorSegmentCache.clear();vectorSegmentLoads.clear();updateHistoryButtons();updateItemClipboardButtons();
...
function appendMeasurementGraphic(svg,annotation,factor,draft=false){
  ...
  if(!draft){group.dataset.id=annotation.id;group.addEventListener("pointerdown",event=>startMeasurementMove(event,annotation.id));group.addEventListener("click",event=>{event.preventDefault();event.stopPropagation();selectedControlPoint=null;if(!applyFormatPainter(annotation.id))selectMeasurement(annotation.id);});}
  svg.append(group);
}
...
function selectMeasurement(id){
  ...
  const hasAreaProperties=["area","diameter"].includes(a.measureKind);$("measurementPerimeterField").hidden=!hasAreaProperties;$("measurementAreaFields").hidden=!hasAreaProperties;syncMeasurementAreaControls(a);
  showControlPointHint(a);
}
...
els.shell.addEventListener("pointerdown",event=>{if(!state.tool.startsWith("markup-")||event.button!==0)return;const kind=state.tool.slice(7);if(kind==="polygon")return;event.preventDefault();event.stopPropagation();const point=measurementPointFromEvent(event);if(kind==="flag"){const factor=state.scale*1.25,pageSize={width:els.shell.clientWidth/factor,height:els.shell.clientHeight/factor};suppressMarkupClick=true;addMarkup("flag",defaultFlagPoints(point,pageSize));return;}markupDraft={kind,points:[point],hover:point,pointerId:event.pointerId};els.shell.setPointerCapture(event.pointerId);renderMarkupDraft();},true);
...
function updateFormatPainterUi(){const button=$("formatPainterButton");button.classList.toggle("format-painter-active",formatPainter.active);button.setAttribute("aria-pressed",String(formatPainter.active));els.shell.classList.toggle("format-painter-active",formatPainter.active);updateItemClipboardButtons();}
...
function startFormatPainter(){const source=state.annotations.find(item=>item.id===state.selectedId&&(item.type==="markup"||item.type==="measurement"&&item.measureKind!=="calibration"));if(!source)return;formatPainter.active=true;formatPainter.source=structuredClone(source);updateFormatPainterUi();toast("Format Painter active. Press Esc to stop.");}
function applyFormatPainter(targetId){if(!formatPainter.active)return false;const target=state.annotations.find(item=>item.id===targetId);if(!target||target.id===formatPainter.source?.id)return true;const patch=formatPainterPatch(formatPainter.source,target);if(!patch){toast("Choose a compatible markup or measurement.");return true;}snapshot();Object.assign(target,patch);renderAnnotations();markChanged();queueThumbnailRefresh(target.pageId);target.type==="measurement"?selectMeasurement(target.id):selectMarkup(target.id);toast("Format applied. Press Esc to stop.");return true;}
...
for(const pair of [["measurementLinePresets","measurementLineColor"],["measurementLabelPresets","measurementLabelColor"],["measurementShadePresets","measurementShadeColor"],["markupLinePresets","markupLineColor"],["markupFillPresets","markupFillColor"],["markupFlagTextPresets","markupFlagTextColor"]])bindPresetPicker(...pair);
...
function drawCanvasMarkup(ctx,a,scale){
  ...
  if(!["line","arrow","freehand"].includes(kind)&&opacity>0){ctx.save();ctx.globalAlpha=opacity;path?ctx.fill(path):ctx.fill();ctx.restore();}path?ctx.stroke(path):ctx.stroke();
...
function drawVectorMarkup(page,a,font){
  const points=a.points||[];if(points.length<2)return;const{height}=page.getSize(),pdfPoints=points.map(point=>({x:point.x,y:height-point.y})),kind=a.markupKind,bounds=markupBounds(points),pdfBounds={x:bounds.x,y:height-bounds.y-bounds.h,w:bounds.w,h:bounds.h},strokeValue=hexToRgb(a.strokeColor||"#d04a3a"),fillValue=hexToRgb(a.fillColor||"#fff2a8"),stroke=PDFLib.rgb(strokeValue.r,strokeValue.g,strokeValue.b),fill=PDFLib.rgb(fillValue.r,fillValue.g,fillValue.b),width=a.strokeWidth||2,line={color:stroke,thickness:width,dashArray:measurementLineDash(a.lineType||"solid")},drawSegment=(first,last)=>page.drawLine({start:first,end:last,...line});
  if(kind==="ellipse")page.drawEllipse({x:bounds.x+bounds.w/2,y:height-bounds.y-bounds.h/2,xScale:bounds.w/2,yScale:bounds.h/2,color:fill,opacity:a.fillOpacity??0,borderColor:stroke,borderWidth:width,borderDashArray:line.dashArray});
  else if(kind==="cloud")page.drawSvgPath(cloudPath(pdfBounds,"pdf"),{color:fill,opacity:a.fillOpacity??0,borderColor:stroke,borderWidth:width});
  else if(kind==="rectangle")page.drawRectangle({x:pdfBounds.x,y:pdfBounds.y,width:pdfBounds.w,height:pdfBounds.h,color:fill,opacity:a.fillOpacity??0,borderColor:stroke,borderWidth:width,borderDashArray:line.dashArray});
  else if(kind==="flag"){const flag=flagPolygonPoints(bounds).map(point=>({x:point.x,y:height-point.y})),path=flag.map((point,index)=>`${index?"L":"M"} ${point.x} ${point.y}`).join(" ")+" Z";page.drawSvgPath(path,{color:fill,opacity:a.fillOpacity??1,borderColor:stroke,borderWidth:width});if(a.showFlagText!==false){const notch=Math.min(bounds.w*.28,bounds.h*.55),left=bounds.x+notch,right=bounds.x+bounds.w,size=a.fontSize||14,lineHeight=size*1.2,lines=wrapTextForPdf(a.text||"Flag",font,size,Math.max(8,right-left-8)),contentHeight=lines.length*lineHeight,vertical=a.verticalAlign||"middle",bottom=height-bounds.y-bounds.h,firstY=vertical==="top"?height-bounds.y-size-4:vertical==="bottom"?bottom+4+(lines.length-1)*lineHeight:bottom+(bounds.h-contentHeight)/2+contentHeight-size,colorValue=hexToRgb(a.textColor||"#ffffff"),textColor=PDFLib.rgb(colorValue.r,colorValue.g,colorValue.b),align=a.textAlign||"center";for(const [index,text] of lines.entries()){const textWidth=font.widthOfTextAtSize(text,size),x=align==="left"?left+4:align==="right"?right-4-textWidth:(left+right-textWidth)/2,y=firstY-index*lineHeight;page.drawText(text,{x,y,size,font,color:textColor});if(a.textUnderline)page.drawLine({start:{x,y:y-1.5},end:{x:x+textWidth,y:y-1.5},color:textColor,thickness:Math.max(.6,size/16)});}}}
  else{if(kind==="polygon"){const path=pdfPoints.map((point,index)=>`${index?"L":"M"} ${point.x} ${point.y}`).join(" ")+" Z";page.drawSvgPath(path,{color:fill,opacity:a.fillOpacity??0,borderColor:stroke,borderWidth:width});}for(let index=1;index<pdfPoints.length;index++)drawSegment(pdfPoints[index-1],pdfPoints[index]);if(kind==="polygon")drawSegment(pdfPoints.at(-1),pdfPoints[0]);if(kind==="arrow"){drawVectorArrowhead(page,pdfPoints[0],pdfPoints[1],a.startArrow||"none",stroke,width);drawVectorArrowhead(page,pdfPoints.at(-1),pdfPoints.at(-2),a.endArrow||"filled",stroke,width);}}
}
function drawVectorAnnotation(page,a,fonts){
  ...
  if(a.type==="markup"){drawVectorMarkup(page,a,fonts[standardFontKey(a)]);return;}
  ...
  lines.forEach((line,index)=>{const lineWidth=font.widthOfTextAtSize(line,size),lineX=x+Math.max(0,w-lineWidth)*horizontalFactor,lineY=firstY-index*lineHeight,textColor=PDFLib.rgb(c.r,c.g,c.b);page.drawText(line,{x:lineX,y:lineY,size,font,color:textColor});if(a.textUnderline)page.drawLine({start:{x:lineX,y:lineY-1.5},end:{x:lineX+lineWidth,y:lineY-1.5},color:textColor,thickness:Math.max(.6,size/16)});});
}
