...
export function createLayer(name, id = crypto.randomUUID()) {
  ...
  return { id: String(id), name: cleanName, visible: true };
}
...
export function normalizeLayers(layers = []) {
  ...
  for (const value of layers) {
    ...
    ids.add(id);names.add(nameKey);result.push({ id, name, visible: value.visible !== false });
  }
  return result;
}

export function deriveLayersFromAnnotations(annotations = []) {
  return normalizeLayers(annotations.filter(item => item?.layerId && item?.layerName).map(item => ({ id: item.layerId, name: item.layerName, visible: item.layerVisible !== false })));
}
