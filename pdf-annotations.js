...
function annotationAppearance(PDFLib, page, spec) {
  const context=page.doc.context,rect=spec.rect,layout=appearanceLayout(rect,spec.appearanceRotation||0),width=layout.width,height=layout.height,stroke=spec.color||[0,0,0],fill=spec.interiorColor,lineWidth=Math.max(.25,spec.width||1),commands=["q",`1 0 0 1 ${numberText(rect[0])} ${numberText(rect[1])} cm`,`${layout.matrix.map(numberText).join(" ")} cm`,`${colorText(stroke)} RG`,`${colorText(fill||stroke)} rg`,`${numberText(lineWidth)} w`,"1 J 1 j"];
...
export function pdfAnnotationSpec(annotation, pageGeometry, measurementLabel = "") {
  ...
  if(spec){spec.appearanceRotation=geometry.rotation;if(spec.subtype==="FreeText")spec.rotation=geometry.rotation;}
  return spec;
}
...
export function parseBluebeamKillerAnnotationData(value){if(typeof value!=="string"||!value||value.length>MAX_STORED_ANNOTATION_BYTES)return null;try{const payload=JSON.parse(value),annotation=payload?.annotation,finite=value=>Number.isFinite(Number(value)),validPoint=point=>point&&finite(point.x)&&finite(point.y),validBox=item=>finite(item.x)&&finite(item.y)&&finite(item.w)&&finite(item.h);if(payload?.version!==BLUEBEAM_KILLER_DATA_VERSION||!annotation||typeof annotation!=="object"||Array.isArray(annotation)||!["text","highlight","markup","measurement"].includes(annotation.type))return null;if(annotation.type==="markup"&&(!["line","arrow","rectangle","ellipse","cloud","polygon","freehand","flag","callout","legend"].includes(annotation.markupKind)||!Array.isArray(annotation.points)||annotation.points.length>10_000||!annotation.points.every(validPoint)||annotation.markupKind==="callout"&&!validBox(annotation)))return null;if(annotation.type==="measurement"&&(!["length","polyline","area","perimeter","diameter","angle","count","calibration"].includes(annotation.measureKind)||!Array.isArray(annotation.points)||annotation.points.length>10_000||!annotation.points.every(validPoint)))return null;if(annotation.type==="text"&&!validBox(annotation))return null;if(annotation.type==="highlight"){const rects=annotation.rects?.length?annotation.rects:[annotation];if(rects.length>10_000||!rects.every(validBox))return null;}return annotation;}catch{return null;}}
...
export function addPdfLibAnnotation(PDFLib, page, annotation, measurementLabel = "") {
  ...
  const dateValue = new Date().toISOString().replace(/[-:T]/g, "").replace(/\.\d{3}Z$/, "Z"),pdfDate=PDFLib.PDFString.of(`D:${dateValue}`),numberFormat = (unit, factor) => context.obj({ Type: name("NumberFormat"), U: text(unit), C: PDFLib.PDFNumber.of(factor), D: PDFLib.PDFNumber.of(100), F: name("D"), SS: text("") });
  ...
  const data = { Type: name("Annot"), Subtype: name(spec.subtype), Rect: context.obj(spec.rect), P: page.ref, F: PDFLib.PDFNumber.of(4), NM: PDFLib.PDFString.of(String(spec.id || crypto.randomUUID())), T: text(spec.title), Subj: text(spec.subject), Contents: text(spec.contents), CreationDate:pdfDate, M:pdfDate, CA: PDFLib.PDFNumber.of(spec.opacity ?? 1), [BLUEBEAM_KILLER_DATA_KEY]:text(bluebeamKillerAnnotationData(annotation)) };if(spec.color)data.C=context.obj(spec.color);
