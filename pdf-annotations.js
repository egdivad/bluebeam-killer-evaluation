...
export function addPdfLibAnnotation(PDFLib, page, annotation, measurementLabel = "") {
  ...
  const data = { Type: name("Annot"), Subtype: name(spec.subtype), Rect: context.obj(spec.rect), P: page.ref, F: PDFLib.PDFNumber.of(4), NM: text(spec.id || crypto.randomUUID()), T: text(spec.title), Subj: text(spec.subject), Contents: text(spec.contents), M: PDFLib.PDFString.of(`D:${dateValue}`), CA: PDFLib.PDFNumber.of(spec.opacity ?? 1), [BLUEBEAM_KILLER_DATA_KEY]:text(bluebeamKillerAnnotationData(annotation)) };if(spec.color)data.C=context.obj(spec.color);
