export function shortcutCommand(event) {
  ...
  if (ctrl && shift && !alt) {
    ...
    return ({ c: "format-painter", d: "delete-page", f: "select-by-properties", g: "ungroup", n: "insert-page", s: "export" })[key] || null;
  }
  if (ctrl && !shift && !alt) {
    if (key === "arrowleft") return "previous-page";
    if (key === "arrowright") return "next-page";
    return ({ "0": "fit-width", "4": "layout-single", "5": "layout-continuous", "6": "layout-side", "7": "layout-continuous-side", "8": "actual-size", "9": "fit-page", f: "search", g: "group", o: "open", s: "export", y: "redo", z: "undo" })[key] || null;
  }
  if (ctrl || alt) return null;
  if (["arrowleft", "arrowright", "arrowup", "arrowdown"].includes(key)) return `move-selected-${key.slice(5)}`;
